...
def get_lightning_html(power_kw, status_color):
    # CLUSTER: 50-200 (1 Blau), 201-300 (2 Rot), >300 (3 Schwarz)
    if power_kw > 300:
...
st.sidebar.markdown(f'''<div style="background-color: #f0f0f0; padding: 15px; border-radius: 10px; border: 1px solid #ccc; color: #000;">
    ...
    <i class="fa fa-bolt" style="color:#3b82f6;"></i> 50-200kW | <i class="fa fa-bolt" style="color:#ef4444;"></i><i class="fa fa-bolt" style="color:#ef4444;"></i> 201-300kW | <i class="fa fa-bolt" style="color:#000;"></i><i class="fa fa-bolt" style="color:#000;"></i><i class="fa fa-bolt" style="color:#000;"></i> >300kW
...
# --- DATEN LADEN & PRÄZISE FILTERN ---
found_count = 0
if API_KEY:
    try:
        # Wir laden im festen 250km Radius, um API-Lücken zu vermeiden
        params = {"key": API_KEY, "latitude": final_lat, "longitude": final_lon, "distance": 250, "distanceunit": "KM", "maxresults": 500, "compact": "false", "connectiontypeid": "33,30"}
        ...
        for poi in res:
            ...
            # FILTER 1: Liegt die Station wirklich im blauen Reichweiten-Kreis?
            dist = haversine(final_lat, final_lon, lat, lon)
            if dist > range_km: continue
            ...
            # FILTER 2: Mindestleistung & Tesla
            ...
            pop_html = f'<div style="width:180px; color:black;"><b>{int(max_pwr)} kW</b> ({qty} 🔌)<br>{op_name}<br><div style="display:flex; gap:5px; margin-top:8px;"><a href="http://maps.google.com/?daddr={lat},{lon}" target="_blank" style="background:#4285F4; color:white; padding:5px; border-radius:3px; text-decoration:none; flex:1; text-align:center;">Google</a><a href="http://maps.apple.com/?daddr={lat},{lon}" target="_blank" style="background:black; color:white; padding:5px; border-radius:3px; text-decoration:none; flex:1; text-align:center;">Apple</a></div></div>'
...
st_folium(m, height=800, width=None, use_container_width=True, key="super_stable_v1")
